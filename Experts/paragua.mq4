...
int OnInit() {
   ...
   // 1. CARGAR DATOS PERSISTENTES (Recuperación tras reinicio)
   LoadPersistentData();
   
   // 2. INICIALIZAR TRACKERS (Para que el panel muestre datos de inmediato)
   double eq = (AccountBalance() > 0) ? (AccountEquity() / AccountBalance()) * 100.0 : 100.0;
   UpdateHistoricalTrackers(eq, MarketInfo(Symbol(), MODE_SPREAD));

   // ESTA LÍNEA ES LA QUE CREA EL PANEL AL PRINCIPIO
   DeleteMonitoringPanel(); // Limpia restos antiguos
   CreateMonitoringPanel();
   
   EventSetTimer(TimerInterval);
   Print("Protector Iniciado. Símbolo: ", TradingSymbol, " | Datos cargados.");
...
//+------------------------------------------------------------------+
//| Función de evento Tick (Motor Principal)                         |
//| Controla el flujo: Vigilia -> Protección -> Agotamiento -> Cierre|
//+------------------------------------------------------------------+
void OnTick()
{
   // 1. Obtención de datos básicos de cuenta
   double balance = AccountBalance();
   double equity  = AccountEquity();
   double eqPercent = (balance > 0) ? (equity / balance) * 100.0 : 100.0;
   double spread = GetSpreadForXAUUSD();

   // 2. Actualizar trackers de panel y variables globales
   ...
   {
      // --- ESTADO: VIGILIA ---
      // Monitorea si el Equity cae por debajo del umbral para activar Paragua
   ...
   else
   {
      ...
      // B. Monitoreo de Agotamiento (El "Cuándo")
      // Revisa si el precio se ha estancado por 30 segundos (Agotamiento_Confirmado)
      DeterminarNivelesReferencia();

      // C. Motor de Ejecución de Cierre (Balanza del 35%)
      // Solo intenta cerrar si DeterminarNivelesReferencia() confirmó agotamiento
      ...
      // Si el Equity vuelve a la normalidad sin necesidad de cierres forzados
      VerificarRecuperacionEquity(eqPercent);
   }

   CalcularDistanciaOperativa();

   // 4. Actualización Visual (Refresco de etiquetas en tiempo real)
   if(Counter % 10 == 0) // Optimización: actualizar panel cada 10 ticks
   {
...
void CheckActivationConditions(double equityPercent)
{
   ...
   // Si el equity cae por debajo del umbral...
   if(!InWaitingState)
   {
...
int CountPrincipalPositions()
{
   ...
   {
      if(OrderSelect(i, SELECT_BY_POS)) {
         // Verificamos si la orden abierta es de ORO (XAU o GOLD)
...
void CalcularDistanciaOperativa()
{
   ...
   for(int i = OrdersTotal()-1; i >= 0; i--)
   {
      if(OrderSelect(i, SELECT_BY_POS)) 
      {
         // Filtro simplificado: Que sea ORO y que NO sea del Paragua
         if(NormalizeSymbol(OrderSymbol()) == "XAU_FOUND" && OrderMagicNumber() != Magic_Number)
         {
...
{
   // Mantenemos tu lista original pero agregamos XAUUSDp al principio
   string possibleSymbols[] = {"XAUUSDp", "XAUUSD", "XAUUSD.", "GOLD", "XAUUSDm"};
   
   for(int i = 0; i < ArraySize(possibleSymbols); i++)
   {
      // SymbolSelect con 'true' asegura que el símbolo se agregue al Market Watch si no está
   ...
   // El "Último recurso" que tenías, pero adaptado a tu cuenta actual
...
bool AbrirCoberturaConReintentos()
{
   ...
   // --- CAMBIO CRÍTICO: Obtener precios Bid/Ask específicos del ORO ---
...
{
    // Pedimos los datos específicamente del símbolo de trading detectado (XAUUSDp)
...
// Función que cierra solo órdenes del Principal (las que NO tienen tu Magic Number)
...
// Función que equilibra la balanza cuando el Paragua ya puede cerrar (Fase > 35%)
void EquilibrarBalanza()
{
   // Buscamos cerrar la orden del Paragua más antigua que esté en profit
   for(int i = OrdersTotal()-1; i >= 0; i--)
   {
...
{
   // Usamos GetPrincipalTotalLot() que es la que ya existe en tu código
